...
export const experiences = [
  {
    ...
    details: [
      "Building an AI framework that assembles production-ready React interfaces from natural-language input — an agent layer that selects, composes, and wires vetted components into a working site in seconds, rather than generating code from scratch",
      "Building agent orchestration and an evaluation harness on the Claude API, with React, Next.js and TypeScript",
      "Designing the component-selection and retrieval layer so output is deterministic, type-safe, and accessible rather than hallucinated markup",
      "Applying a security-first review pipeline to generated code (dependency, secrets, and injection checks)",
      "In active development — public launch coming soon",
    ],
    skills: [
      "TypeScript",
      "Next.js",
  ...
  },
  {
    ...
    details: [
      "Developed scalable backend services in Go and built the modern, responsive React and TypeScript interfaces that clinical staff work in day to day",
      "Built AI-powered features across the Scribe and EMR products used by clinics throughout Canada, turning recorded consultations into structured clinical documentation a clinician reviews before signing off",
      "Integrated LLM providers behind a server-side layer, so model credentials and patient data never reached the browser",
      "Architected secure distributed systems — firewall configuration, AWS account and network hardening, and tightened access boundaries between services",
      "Designed compliance-aware healthcare infrastructure, building access control and data-handling boundaries around Canadian patient-privacy requirements",
    ],
    skills: [
      "Go",
      "React",
  ...
  },
  {
    ...
    details: [
      "Joined as founding engineer and built the AI scribe end to end — streaming consultation audio to Deepgram for real-time speech-to-text, then using OpenAI to turn the transcript into structured clinical documentation",
      "Built the product on Next.js, including authentication, consultation history, and a review step so clinicians could edit and approve every generated note",
      "Architected and deployed the full cloud-native system on AWS EKS, containerizing services and running them on Kubernetes",
      "Developed secure APIs for real-time medical transcription workflows, keeping provider credentials and audio off the client",
      "Owned product and infrastructure alike, taking the scribe from prototype to a deployed, demoable service inside a three-month build",
    ],
    skills: [
      "Next.js",
      "Deepgram",
  ...
  },
  {
    ...
    details: [
      ...
      "Built the training system in PyTorch with Flower for FedAvg aggregation and Opacus for per-sample gradient clipping and calibrated Gaussian noise, evaluated across simulated clients on MNIST",
      "Analyzed four attack surfaces — data breach, membership inference, model inversion, and linkage — and mapped each to the technique that mitigates it",
      "Published the findings as a self-contained interactive web report with a live noise slider demonstrating the privacy/accuracy trade-off",
    ],
    skills: [
      "Python",
      "PyTorch",
  ...
  },
  {
    ...
    details: [
      "Led backend API development in Node.js, owning the design and delivery of the service layer behind the product",
      "Migrated legacy PHP services to Node.js and cut response time by 25%, moving endpoints across incrementally so existing consumers kept working throughout",
      "Improved overall system performance by 20% through refactoring, query tuning, and removing redundant work from hot paths",
      "Standardized API contracts and error handling across services, so new endpoints shipped against one consistent pattern",
    ],
    skills: [
      "Node.js",
      "PHP",
  ...
  },
  {
    ...
    details: [
      "Full-stack engineer across web and React Native, shipping features to the Jakarta co-living platform on both the site and the mobile app",
      "Reduced technical debt by migrating legacy parts of the system onto a modern stack, retiring the older implementation as each piece moved over",
      "Improved organic reach through Next.js rendering optimizations and automated sitemap generation across listing pages",
      "Worked directly with product and design in a remote, cross-timezone team to turn requirements into shipped features",
    ],
    skills: [
      "React Native",
      "Next.js",
  ...
  },
  {
    ...
    details: [
      "Led the development team building full-stack web applications, taking projects from requirements through to delivery",
      "Owned technical architecture decisions — stack selection, data modelling, and how the frontend and backend talked to each other",
      "Coordinated deliverables and timelines across the team, and reviewed work before it shipped to clients",
      "Mentored junior developers through code review, pairing on tricky features and raising the baseline of what the team could ship alone",
    ],
    skills: [
      "Team Leadership",
      "Full-Stack",
  ...
  },
  {
    ...
    details: [
      ...
      "Built the CMS from scratch so users and admins could edit profiles, update UI content, and track payments without touching code",
      "Shipped every product surface in one app: e-commerce book shop, blogging system, portfolio pages, and monthly/yearly membership billing",
      "Built the Node.js and Express REST API on MongoDB, with a dedicated IAM layer and session-limited login guarding admin access",
      "Handled design, deployment, and maintenance — Sass-styled frontend on a Linux server behind NGINX, still live in production",
    ],
    skills: [
      "Node.js",
      "Express",
